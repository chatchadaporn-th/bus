...
$paymentData= "SELECT * ,count(*) over () as total FROM ticket_run  AS t1 INNER JOIN seat AS t2 ON (t1.ticketid= t2.seat_ticket_run) INNER JOIN paths AS t3 ON (t2.seat_pathid = t3.path_id) INNER JOIN bus_table AS t4 ON (t3.bus_id = t4.bus_id) WHERE t1.seat_id= '".$_POST['select_name']."' AND t1.ticket_tel = '".$_POST['select_tel']."'";
$sqlpaymentData = mysqli_query($conn,$paymentData);
$paymentData = mysqli_fetch_array($sqlpaymentData);
    if(empty($paymentData['bus_type'])){
                        ...
                        <table class="table table-striped ">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center" scope="col">หมายเลขตั๋ว</th>
