            ...
            <div id="content">
                ...
                <div class="container">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800" style="font-weight:bold;"> <i class="fas fa-edit"></i> แก้ไขรถบัส</h1>
                    </div>
                    <!-- Page Heading -->
                    <!-- DataTales Example -->
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">แก้ไขรถบัส</h6>
                        </div>
                        <div class="card-body">
                            ...
                            <form method="POST">
                                ...
                                <div class="form-group">
                                    <label for="exampleInputbus_no">ทะเบียนรถ</label>
                                    <input type="text" class="form-control" id="exampleInputbus_no" name="updatebusno" value="<?= $row['bus_no'] ?>" >
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputbus_type">ประเภทรถ</label>
                                    <input type="text" class="form-control" id="exampleInputbus_type" name="updatebustype" value="<?= $row['bus_type'] ?>" >
                                </div>
                                <button type="submit" name="submitbus" class="btn btn-primary">Submit</button>
                            </form>
                        </div>
                    </div>
                    <?php //อัพเดทเวลาต้นทาง
                    if (isset($_POST['submitbus'])) {
                        $updatebusno = $_POST["updatebusno"];
                        $updatebustype = $_POST["updatebustype"];
                        $busid = $row['bus_id'];
                        $getiddd = $_GET['id'];
                        $updateunit = "UPDATE bus_table SET bus_no = '$updatebusno' , bus_type = '$updatebustype' WHERE bus_id = '$busid'";
                        $$update_unit = mysqli_query($conn, $updateunit) or die("Update start Failed");

                        echo '<script language="JavaScript">
                                swal({
                                title: "Success Update",
                                text: "อัพเดทรถบัสสำเร็จ",
                                button: "OK",
                                icon: "success",
                                });
                                    </script>';

                        echo '<meta http-equiv="refresh" content="1; url=edit_bus.php?id=' . $getiddd . '" />';
                        mysqli_close($conn);
                        exit;
                    }
                    ?>
